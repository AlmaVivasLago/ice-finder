...
import cryo_et_ice_det  # noqa
from cryo_et_ice_det.data.datamodule import MetaData
...
def run(cfg: DictConfig) -> str:
    ...
    # Instantiate the callbacks Freaking ducking save in zipwith proper metadata
    ...
    # # logger = hydra.utils.instantiate(cfg.train.logging)
    logger = TensorBoardLogger(
        save_dir=PROJECT_ROOT / "logs",
        name = cfg.core.name)
    
    cfg.train.callbacks[0]['dirpath'] = logger.log_dir + "/ckpt"
    ...
    # os.mkdir(logger.log_dir + "/ckpt")
    # cfg.train.logging.logger.save_dir = logger.log_dir + "/ckpt"
    #trainer.ckpt_path =  cfg.train.callbacks[0]['dirpath']
